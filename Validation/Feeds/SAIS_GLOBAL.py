#!/usr/bin/env python3
"""Standalone SAIS_GLOBAL processor: parse -> normalize -> correlate -> enrich -> XML -> forward."""
...
SOURCE_NAME="SAIS_GLOBAL"
INPUT_TYPE="FOLDER"                         # FOLDER / TCP
INPUT_FOLDER=r"DB_Data\SAIS_GLOBAL"
TCP_HOST="127.0.0.1"; TCP_PORT=10001
OUTPUT_FOLDER=r"XML_Output\SAIS_GLOBAL"
...
logging.basicConfig(level=os.getenv("VALIDATION_LOG_LEVEL","INFO").upper(),format="%(asctime)s %(levelname)s SAIS_GLOBAL %(message)s",
                    handlers=[logging.StreamHandler(),logging.FileHandler(LOGS/"SAIS_GLOBAL.log",encoding="utf-8")])
log=logging.getLogger("SAIS_GLOBAL")
DONE_FILE=STATE/"SAIS_GLOBAL_processed.json"
...
def process_records(records,msg,ref,conn):
    ...
    for i,r in enumerate(records,1):
        try:
            ...
            x=xml(l);name=re.sub(r"[^A-Za-z0-9_.-]+","_",f"SAIS_GLOBAL_{msg}_{i}_{l.get('id.mmsi') or 'unknown'}");p=OUT/(name+"_"+hashlib.sha256(x.encode()).hexdigest()[:16]+".xml")
...
def process_file(p,done,ref,conn):
    try:
        ...
        if h in done:return
        lines=p.read_text(encoding="utf-8",errors="replace").splitlines();records=parse_lines(lines);n=process_records(records,p.stem,ref,conn);done.add(h);DONE_FILE.write_text(json.dumps(sorted(done)),encoding="utf-8");log.info("Processed %s: records=%d xml=%d",p.name,len(records),n)
