#!/usr/bin/env python3
"""Standalone LRIT processor: parse -> normalize -> correlate -> enrich -> XML -> forward."""
...
INPUT_FOLDER=r"DB_Data\LRIT"
TCP_HOST="127.0.0.1"; TCP_PORT=10001
OUTPUT_FOLDER=r"XML_Output\LRIT"
...
logging.basicConfig(level=os.getenv("VALIDATION_LOG_LEVEL","INFO").upper(),format="%(asctime)s %(levelname)s LRIT %(message)s",
                    handlers=[logging.StreamHandler(),logging.FileHandler(LOGS/"LRIT.log",encoding="utf-8")])
log=logging.getLogger("LRIT")
DONE_FILE=STATE/"LRIT_processed.json"
...
def parse_lrit(lines):
    out=[]
    reader=csv.reader(lines)
    for line_no,cols in enumerate(reader,1):
        if not cols: continue
        if line_no==1 and cols[0].strip().lower()=="mmsi": continue
        if len(cols)<13: raise ValueError(f"LRIT line {line_no}: fewer than 13 columns")
        r=R()
        r.mmsi=num(cols[0],True);r.latitude=num(cols[1]);r.longitude=num(cols[2]);r.sog=num(cols[3]);r.cog=num(cols[4]);r.true_heading=num(cols[5]);r.nav_status=num(cols[7],True)
        r.timestamp=clean(cols[8]) if len(cols)>8 and clean(cols[8]) else datetime.now(timezone.utc).isoformat()
        r.vessel_name=clean(cols[11]) if len(cols)>11 else None;r.imo=num(cols[12],True) if len(cols)>12 else None;r.callsign=clean(cols[13]) if len(cols)>13 else None
        r.length=num(cols[14]) if len(cols)>14 else None;r.width=num(cols[15]) if len(cols)>15 else None;r.draught=num(cols[16]) if len(cols)>16 else None;r.destination=clean(cols[17]) if len(cols)>17 else None
        r.vessel_type=clean(cols[18]) if len(cols)>18 else None;r.eta=clean(cols[19]) if len(cols)>19 else None;r.raw_payload=",".join(cols);out.append(r)
    return out
...
def process_records(records,msg,ref,conn):
    ...
    for i,r in enumerate(records,1):
        try:
            ...
            x=xml(l);name=re.sub(r"[^A-Za-z0-9_.-]+","_",f"LRIT_{msg}_{i}_{l.get('id.mmsi') or 'unknown'}");p=OUT/(name+"_"+hashlib.sha256(x.encode()).hexdigest()[:16]+".xml")
...
def process_file(p,done,ref,conn):
    try:
        ...
        if h in done:return
        lines=p.read_text(encoding="utf-8",errors="replace").splitlines();records=parse_lrit(lines);n=process_records(records,p.stem,ref,conn);done.add(h);DONE_FILE.write_text(json.dumps(sorted(done)),encoding="utf-8");log.info("Processed %s: records=%d xml=%d",p.name,len(records),n)
