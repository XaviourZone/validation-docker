#!/usr/bin/env python3
"""Standalone VATMS_EAST processor: parse -> normalize -> correlate -> enrich -> XML -> forward."""
...
SOURCE_NAME="VATMS_EAST"
INPUT_TYPE="FOLDER"                         # FOLDER / TCP
INPUT_FOLDER=r"DB_Data\VATMS_EAST"
TCP_HOST="127.0.0.1"; TCP_PORT=10001
OUTPUT_FOLDER=r"XML_Output\VATMS_EAST"
...
logging.basicConfig(level=os.getenv("VALIDATION_LOG_LEVEL","INFO").upper(),format="%(asctime)s %(levelname)s VATMS_EAST %(message)s",
                    handlers=[logging.StreamHandler(),logging.FileHandler(LOGS/"VATMS_EAST.log",encoding="utf-8")])
log=logging.getLogger("VATMS_EAST")
DONE_FILE=STATE/"VATMS_EAST_processed.json"
...
def parse_vatms_east(lines):
    # VATMS EAST uses !WSVDM AIS sentences. !AIVDM is accepted as a compatible
    # AIS sentence when present in the feed capture.
    return parse_lines([x for x in lines if x.strip().startswith(("!WSVDM","!AIVDM"))])
...
def logical(r,receipt,effective):
    ...
    "kinematic.speed":float(r.sog)*0.514444 if r.sog is not None else None,"sys.source.id":245,"sys.track.number":num(r.mmsi,True),"timestamp.receipt":receipt,"timestamp.source":iso_ms(r.timestamp),
...
def process_records(records,msg,ref,conn):
    ...
    for i,r in enumerate(records,1):
        try:
            ...
            x=xml(l);name=re.sub(r"[^A-Za-z0-9_.-]+","_",f"VATMS_EAST_{msg}_{i}_{l.get('id.mmsi') or 'unknown'}");p=OUT/(name+"_"+hashlib.sha256(x.encode()).hexdigest()[:16]+".xml")
...
def process_file(p,done,ref,conn):
    try:
        ...
        if h in done:return
        lines=p.read_text(encoding="utf-8",errors="replace").splitlines();records=parse_vatms_east(lines);n=process_records(records,p.stem,ref,conn);done.add(h);DONE_FILE.write_text(json.dumps(sorted(done)),encoding="utf-8");log.info("Processed %s: records=%d xml=%d",p.name,len(records),n)
