...
INPUT_FOLDER=r"DB_Data\SAIS_IOR"
TCP_HOST="127.0.0.1"; TCP_PORT=10001
OUTPUT_FOLDER=r"XML_Output\SAIS_IOR"
...
logging.basicConfig(level=os.getenv("VALIDATION_LOG_LEVEL","INFO").upper(),format="%(asctime)s %(levelname)s SAIS_IOR %(message)s",
                    handlers=[logging.StreamHandler(),logging.FileHandler(LOGS/"SAIS_IOR.log",encoding="utf-8")])
log=logging.getLogger("SAIS_IOR")
DONE_FILE=STATE/"SAIS_IOR_processed.json"
...
def process_records(records,msg,ref,conn):
    ...
    for i,r in enumerate(records,1):
        try:
            ...
            x=xml(l);name=re.sub(r"[^A-Za-z0-9_.-]+","_",f"SAIS_IOR_{msg}_{i}_{l.get('id.mmsi') or 'unknown'}");p=OUT/(name+"_"+hashlib.sha256(x.encode()).hexdigest()[:16]+".xml")
