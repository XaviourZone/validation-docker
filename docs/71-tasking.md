...
## Release gate
The post-hardening validation gate has now passed **12/12** in GitHub Actions on the current branch (run 94, job 105874005822). It includes Python compile, parser/source/semantic/control-message tests, reference DB importer tests, Web Console configuration tests, XML field coverage tests, full Router → Parser → Forwarder acceptance, Router lifecycle/recovery tests, Docker Compose syntax, Docker image build, offline image save/load, deployed Compose stack restart/recovery, and release-document consistency.

Operational WRS/PANS/NSC databases and representative production datasets are intentionally not committed to this repository. The repository-side importer, lookup/fallback mechanics and deterministic reference fixtures are validated, but authoritative production reference-data coverage still requires the actual deployment databases. No production enrichment result is inferred from the fixture.
