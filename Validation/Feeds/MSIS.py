#!/usr/bin/env python3
"""Standalone MSIS processor: parse -> normalize -> correlate -> enrich -> XML -> forward."""
...
INPUT_FOLDER=r"DB_Data\MSIS"
TCP_HOST="127.0.0.1"; TCP_PORT=10001
OUTPUT_FOLDER=r"XML_Output\MSIS"
...
logging.basicConfig(level=os.getenv("VALIDATION_LOG_LEVEL","INFO").upper(),format="%(asctime)s %(levelname)s MSIS %(message)s",
                    handlers=[logging.StreamHandler(),logging.FileHandler(LOGS/"MSIS.log",encoding="utf-8")])
log=logging.getLogger("MSIS")
DONE_FILE=STATE/"MSIS_processed.json"
...
def logical(r,receipt,effective):
    ...
    "kinematic.speed":float(r.sog)*0.514444 if r.sog is not None else None,"sys.source.id":250,"sys.track.number":num(r.mmsi,True),"timestamp.receipt":receipt,"timestamp.source":iso_ms(r.timestamp),
...
def process_records(records,msg,ref,conn):
    ...
    for i,r in enumerate(records,1):
        try:
            ...
            x=xml(l);name=re.sub(r"[^A-Za-z0-9_.-]+","_",f"MSIS_{msg}_{i}_{l.get('id.mmsi') or 'unknown'}");p=OUT/(name+"_"+hashlib.sha256(x.encode()).hexdigest()[:16]+".xml")
